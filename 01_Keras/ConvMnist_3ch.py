...
class ConvMnist:
	...
	def train(self):
		...
		x_train = np.empty((x_train_org.shape[0], x_train_org.shape[1], x_train_org.shape[2], 3))
		x_train[:,:,:,0] = x_train_org
		x_train[:,:,:,1] = x_train_org
		x_train[:,:,:,2] = x_train_org
		x_test = np.empty((x_test_org.shape[0], x_test_org.shape[1], x_test_org.shape[2], 3))
		x_test[:,:,:,0] = x_test_org
		x_test[:,:,:,1] = x_test_org
		x_test[:,:,:,2] = x_test_org
		x_train = x_train / 255.
...
if __name__ == '__main__':
	...
	conv_mnist.train()
	conv_mnist.save_trained_model(filename='conv_mnist_3ch.h5')
